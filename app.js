...
request = new Request(
                        ...
                        builder.Prompts.choice(session, "Displaying available LOB", LOBS,{ listStyle: builder.ListStyle.button });
                        }
                );
connection1.execSql(request);
}
    ...
    function (session, results) {
       ...
       if (results.response) {
            ...
            session.userData.LOBValue = obj.value;
...
var tempcard = new builder.HeroCard(session)
.buttons([
builder.CardAction.imBack(session, "Display available campaign for product", row[1].value)
]);
...
builder.Prompts.choice(session, "Below products are available for "+session.userData.LOBValue+" line of business", Products,{ listStyle: builder.ListStyle.button });
}
);
connection1.execSql(request);
}
...
]).triggerAction({ matches: /(Display|show)\s.*report/i });;
